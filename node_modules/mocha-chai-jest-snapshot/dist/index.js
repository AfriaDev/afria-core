...
const jestSnapshotPlugin = (optionalConfig) => {
    var _a, _b;
    const packageJson = find_package_json_1.default(process.cwd()).next();
    const rootDir = packageJson.done
        ? process.cwd()
        : path_1.default.dirname(packageJson.filename);
    const config = jest_config_helper_1.replaceRootDirInObject(rootDir, Object.assign(Object.assign(Object.assign({ rootDir }, (_a = packageJson.value) === null || _a === void 0 ? void 0 : _a.jest), helper_1.readJestConfig(rootDir)), optionalConfig));
    // Jest tests snapshotSerializers in order preceding built-in serializers.
    // Therefore, add in reverse because the last added is the first tested.
    (_b = config.snapshotSerializers) === null || _b === void 0 ? void 0 : _b.concat().reverse().forEach((path) => {
        jest_snapshot_1.addSerializer(require.main.require(path));
    });
    _manager = new manager_1.default({
        rootDir,
        snapshotResolver: jest_snapshot_1.buildSnapshotResolver(config),
    });
