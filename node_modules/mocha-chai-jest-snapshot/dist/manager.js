...
class SnapshotManager {
    constructor({ rootDir, snapshotResolver, }) {
        this.snapshotState = null;
        this.snapshotSummary = jest_test_result_helper_1.makeEmptySnapshotSummary(helper_1.snapshotOptions);
        this.context = null;
        this.testFile = "";
    ...
    saveSnap() {
        if (!this.testFile || !this.snapshotState)
            return;
        const packedSnapshotState = jest_test_result_helper_1.packSnapshotState(this.snapshotState);
        jest_test_result_helper_1.addSnapshotResult(this.snapshotSummary, packedSnapshotState, this.testFile);
        this.testFile = "";
        this.snapshotState = null;
    }
    report() {
        const outputs = jest_reporters_lite_1.getSnapshotSummaryOutput(this.rootDir, this.snapshotSummary);
        if (outputs.length > 1)
            console.log("\n" + outputs.join("\n"));
    }
}
