...
import { TestAfriaSwapPay } from '../typechain/TestAfriaSwapPay'
...
import { TestAfriaReentrantCallee } from '../typechain/TestAfriaReentrantCallee'
...
describe('AfriaPool', () => {
  ...
  let factory: AfriaFactory
  let pool: MockTimeAfriaPool

  let swapTarget: TestAfriaCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeAfriaPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestAfriaReentrantCallee')
      ).deploy()) as TestAfriaReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestAfriaSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestAfriaSwapPay')
      underpay = (await underpayFactory.deploy()) as TestAfriaSwapPay
