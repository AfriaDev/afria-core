...
import { AfriaPoolSwapTest } from '../typechain/AfriaPoolSwapTest'
...
describe('AfriaPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('AfriaPoolSwapTest')
            const tester = (await testerFactory.deploy()) as AfriaPoolSwapTest
